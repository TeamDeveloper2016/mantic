...
(function (window) {
	...
	Janal.Control.Zebra.Core = Class.extend({
		init: function () {
		...
		/// Detection ///
		findPrinter: function (query, set) {
...
//{ type: 'raw', format: 'image', data: 'assets/img/image_sample_bw.png', options: { language: 'ZPLII' } },
		...
		listUsbDeviceInterfaces: function () {
		...
		listUsbInterfaceEndpoints: function () {
		...
		displayMessage: function (msg, css) {
			if (css == undefined) {
				css = 'alert-info';
			}
		...
		pinMessage: function (msg, id, css) {
			if (css == undefined) {
				css = 'alert-info';
			}
		...
		serialButton: function (serialPort, data) {
			var click = "";
		...
		formatHexInput: function (inputId) {
			var $input = $('#' + inputId);
			var val = $input.val();
	...
	console.info('Janal.Control.Zebra initialized');
})(window);
