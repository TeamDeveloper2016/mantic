...
public class umbrales extends IBaseAttribute implements Serializable {
  ...
  public void doLoad() {
    ...
    try {
      ...
      this.transferencia= (Entity)DaoFactory.getInstance().toEntity("VistaAlmacenesTransferenciasDto", "lazy", params);
        ...
        this.fuentes = (List<Umbral>)DaoFactory.getInstance().toEntitySet(Umbral.class, "TcManticAlmacenesArticulosDto", "umbrales", params);
        params.put("idAlmacen", this.transferencia.toLong("idDestino"));
        this.destinos= (List<Umbral>)DaoFactory.getInstance().toEntitySet(Umbral.class, "TcManticAlmacenesArticulosDto", "umbrales", params);
      } // if  
      else {
    ...
    String regresar         = null;
      ...
      if(this.toCheck()) {
        ...
        if(transaccion.ejecutar(EAccion.PROCESAR)) {
          UIBackingUtilities.execute("janal.back(' gener\\u00F3 la transferencia ', '"+ this.transferencia.toString("consecutivo")+ "');");
        ...
        JsfBase.addMessage("No existe nada que actualizar", ETipoMensaje.ERROR);
		} // try
		catch (Exception e) {
			Error.mensaje(e);
			JsfBase.addMessageError(e);			
  ...
  public String toColor(Umbral row) {
    return Objects.equals(row.getAction(), ESql.UPDATE)? "janal-tr-orange": "";
  }
