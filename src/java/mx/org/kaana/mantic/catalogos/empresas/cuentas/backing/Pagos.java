...
public class Pagos extends IBaseFilter implements Serializable {
  ...
  protected void init() {
    try {
			this.idEmpresa= JsfBase.getFlashAttribute("idEmpresa")== null ? -1L: (Long)JsfBase.getFlashAttribute("idEmpresa");			
      this.attrs.put("idEmpresa", "order by tc_mantic_ventas.consecutivo, tc_mantic_empresas_deudas.registro desc");
      this.attrs.put("idCliente", this.idEmpresa);     
  ...
  public void doLoad() {
    ...
    try {			
      ...
      columns.add(new Columna("registro", EFormatoDinamicos.FECHA_EXTENDIDA));    
			if(!this.idEmpresa.equals(-1L)){
				...
				this.lazyModel = new FormatCustomLazy("VistaEmpresasDto", "pagos", params, columns);
			} // if
			else
	...
	private Map<String, Object> toPrepare() {
	  Map<String, Object> regresar= new HashMap<>();	
		...
		if(!Cadena.isVacio(this.attrs.get("fechaInicio")))
		  sb.append("(date_format(tc_mantic_empresas_deudas.registro, '%Y%c%d')>= '").append(Fecha.formatear(Fecha.FECHA_ESTANDAR, (Date)this.attrs.get("fechaInicio"))).append("') and ");	
		if(!Cadena.isVacio(this.attrs.get("fechaTermino")))
		  sb.append("(date_format(tc_mantic_empresas_deudas.registro, '%Y%c%d')<= '").append(Fecha.formatear(Fecha.FECHA_ESTANDAR, (Date)this.attrs.get("fechaTermino"))).append("') and ");	
    ...
    try {
			...
			if(this.attrs.get("busqueda")!= null && this.attrs.get("busqueda").toString().length()> 3) {
				...
				empresas = UIEntity.build("VistaClientesDto", "findRazonSocial", params, columns, Constantes.SQL_TODOS_REGISTROS);      
	...
	public void doSeleccionado() {
		...
		try {
			listado= (List<UISelectEntity>) this.attrs.get("empresa");
